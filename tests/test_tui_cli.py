"""The cockpit's DB-free surface: the CLI verbs, the actions, the models.

Skipped, not failed, where the ``tui`` extra cannot install (3.10, 3.11) —
except the first test, which is precisely about that case and runs everywhere.
"""
...
def test_a_missing_extra_says_what_to_install_and_exits_2(monkeypatch, capsys):
    ...
    ``None`` in ``sys.modules`` makes ``from . import tui`` raise ImportError,
    which is what a 3.10 interpreter sees; the test runs on every Python.
    """
    import featurizer

    # Both halves matter: ``from . import tui`` returns the package attribute
    # when a previous test already imported the submodule, and only consults
    # ``sys.modules`` (where ``None`` means "import fails") once it is gone.
...
lynkeus = pytest.importorskip("lynkeus")
